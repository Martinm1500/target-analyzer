...
async function iniciarOperacion() {
  ...
  let urlngresada = inputObjetivo.value.trim();
  if (
    !urlngresada.startsWith("https://") &&
    !urlngresada.startsWith("https://")
  ) {
    urlngresada = "https://" + urlngresada;
  }
  try {
    new URL(urlngresada);
  } catch (error) {
  ...
  try {
    const respuesta = await fetch("http://localhost:3000/api/escanear", {
      ...
      body: JSON.stringify({ url: urlngresada }),
    ...
    panelMetricas.innerHTML = `<ul style="list-style: none; padding: 0; margin: 0;"><li>LATENCIA: 
        ...
        TOTAL: <span style="color: var(--color-terminal)">${datos.metricas.pesoDocumntoKb}KB</span></li>
